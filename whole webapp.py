...
def index():
    ...
    cur = conn.cursor()
    cur.execute(
            """SELECT members.username, data.author, data.date, data.latitude, data.longitude, data.litter
...
def create():
    if load_signedin_user():
        if request.method == 'POST' :
            ...
            if error is not None :
                ...
                return redirect(url_for('index'))
                    ...
                    conn = connect(connStr)
                    cur = conn.cursor()
                    cur.execute('INSERT INTO data (date, latitude, longitude, litter, author, author_id) VALUES (%s, %s, %s, %s, %s, %s)', 
                               (date, latitude, longitude, litter, author, g.user[0])
                               )
                    cur.close()
                    conn.commit()
                    conn.close()
        ...
        error = 'Only SignedIn Users Can Add New Cases!'
        flash(error)
        return redirect(url_for('signin'))
...
def get_post(id):
    ...
    if post is None:
        abort(404, "Case Added by {} doesn't exist!".format(id))
    ...
    if load_signedin_user():
        ...
        if request.method == 'POST' :
            ...
            if error is not None :
                ...
                conn = connect(connStr)
                cur = conn.cursor()
                cur.execute('UPDATE data SET date = %s, latitude = %s, longitude = %s, litter = %s'
                               'WHERE author = %s', 
                               (date, latitude, longitude, litter, id)
                               )
                cur.close()
                conn.commit()
                conn.close()
...
def delete(id):
    ...
    cur = conn.cursor()
    cur.execute('DELETE FROM data WHERE author = %s', (id,))
    conn.commit()
    conn.close()
    return redirect(url_for('index'))                               
